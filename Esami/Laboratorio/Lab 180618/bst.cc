...
char* get_nominativo(bst t) {

	char *p = t->inf.Nominativo;
	return p;
...
/** Funzione che permette la stampa in modo crescente delle chaivi e del campo informativo di un albero. */

void stampaCrescente(bst n) {

    if(get_left(n) != NULL) {

        stampaCrescente(get_left(n));

    }
	cout << "Seriale: \t  Nominativo: " << endl;
    print_key(get_key(n));
	cout << "\t \t";
	print(n->inf);
	cout << endl;
